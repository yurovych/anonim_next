...
const ChatItself: React.FC<ChatItselfProps> = ({
  ...
  const [isTypingObj, setIsTypingObj] = useState<{
    isTyping: boolean;
    uId: string;
  }>({ isTyping: false, uId: '' });
  ...
  const { suitableMembers, menUsers, womenUsers } = useMemo(() => {
    ...
    const suitableMembers =
      allUsers?.filter((user) => {
        ...
        return (
          user.uId !== userId &&
          user.userData.sex === interlocutorData.sex &&
          userData.sex === user.interlocutorData.sex &&
          user.userData.age >= interlocutorData.ageFrom! &&
          user.userData.age <= interlocutorData.ageTo! &&
          userData.age! >= user.interlocutorData.ageFrom &&
          userData.age! <= user.interlocutorData.ageTo &&
  ...
  const confirmAddToBlackList = () => {
    ...
    if (matchId) {
      const newUserData = {
        ...userData,
        blackList: [...userData.blackList, matchId],
      };
  ...
  return (
    <>
      {metrics ? (
        <p className={styles.controlAllUsers}>
          control: {Object.keys(metrics?.allUsers)?.length}
        </p>
      ) : (
        ''
      )}
      {modal === MODALS.IS_EXIT ? (
        <ExitModal setModal={setModal} confirm={confirmLeaveChat} />
      ) : (
        ''
      )}
        ...
        {socket?.connected ? (
          <>
              ...
              {!theOneWhoLeft && status !== statusType.disconnected ? (
                <div className={styles.summarySection}>
                  ...
                  <div className={styles.summaryButtons}>
                    <p
                      className={`${styles.generalButton} ${styles.buttonExit}`}
                  ...
                  <div
                    className={`${
                      userId === item.uId
                        ...
                        : styles.anonymChartElement
                    } ${styles.chartElement}`}
                    key={item.createdAt + item.message + index}
                  >
                    <p
                      className={`${styles.messageText} ${item.pending ? styles.pendingMessage : ''}`}
                    >
                      {item.message}
                    </p>
            ...
            {status === statusType.reconnectingProcess ? (
              <div className={styles.leftChatBlock}>
                <p className={styles.leftChatText}>Перепідключення...</p>
              ...
              <div className={styles.leftChatBlock}>
                {DISCONNECT_ON_PURPOSE_REASONS.includes(reason.reason) ? (
                  userId === reason.userId ? (
                    <p className={styles.leftChatText}>Ви покинули чат</p>
                  ) : (
                    <p className={styles.leftChatText}>
                ...
                DISCONNECT_TRANSPORT_CLOSE !== reason.reason ? (
                  <div>
                    ...
                    <p className={styles.leftChatText}>
                      Схоже у{' '}
                      {interlocutorData.sex === 'male' ? 'нього' : 'неї'}{' '}
            ...
            isTypingObj.uId !== userId &&
            socket.connected ? (
              <p className={styles.isTyping}>Щось пишe... 🖊️</p>
            ) : (
              ''
            )}
            {status &&
